...
boolean SevenSegment::begin(const char* driverName, const char* screenMask ){
	...
	for (uint8_t i=0; i < DRIVER_COUNT; i++){
		...
		strcpy(buffer, (char*) (pDriverTable[i].name));

		// Is this driver the one specified?
		if (strcmp(buffer, driverName)==0){

			// Copy driver data into the struct in main memory
			_device.name = (char*) (pDriverTable[i].name);
			_device.segments = (uint8_t) (pDriverTable[i].segments);
			_device.pulseLoad = (boolean) (pDriverTable[i].pulseLoad);
			_device.segmentsOrderInc = (boolean) (pDriverTable[i].segmentsOrderInc);
			_device.initialBit = (boolean) (pDriverTable[i].initialBit);
			_device.cascadable = (boolean) (pDriverTable[i].cascadable);
